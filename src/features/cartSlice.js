...
const cartSlice = createSlice({
  ...
  reducers: {
    addToCart(state, action) {
      ...
      if(itemindex>=0){
        state.Cartitems[itemindex].cartQuantiy+=0.5;
      }else{
         const temp={...action.payload,cartQuantiy:0.5};
         state.Cartitems.push(temp);
         toast.success(`selected Item is added to cart`, {
          position: 'top-center',
        });
    ...
    decreaseCart(state,action){
      ...
      if(state.Cartitems[itemindex].cartQuantiy>0.5){
        state.Cartitems[itemindex].cartQuantiy-=0.5
      }
      else if(stateCcartItem[itemindex].cartQuantiy==0.5){
